# Chat GPT helped optimize this code
    ...
    def optimize(self):
        def objFn(P):
            ...
            if p >= self.Model.p_high:
                # Penalty for p_mid exceeding p_high (e.g., large penalty value)
                penalty += 1e6  # Adjust penalty value as needed
            if p <= self.Model.p_low:
                # Penalty for p_mid below p_low (e.g., large penalty value)
                penalty += 1e6  # Adjust penalty value as needed
            if self.Model.heat_added_2 < 0:
                # Penalty if heat added by second turbine is negative (not physically possible)
                penalty += 1e6  # Adjust penalty value as needed

            # Calculate objective function value (to be minimized)
            objective_value = 100.0 - eff
